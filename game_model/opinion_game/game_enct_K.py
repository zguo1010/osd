...
def main(argv): #100, 0.1, 0.1, 0, 0.8, 0.9, 20
    ...
    utility_at, utility_df, utility_u, utility_h = np.empty([0, 4]), np.empty([0, 2]), np.empty([0, 3]), np.empty(
        [0, 3])
    ...
    draw_community(G, comm_bi, 'results/community_bi_e') #.png
    print(comm_bi_val)
    ...
    draw_community(G, comm_mo, 'results/community_mo_e')
    print(comm_mo_val)
    ...
    if comm_lp_num == 0:
        ...
        draw_community(G, comm_lp, 'results/community_lp_e')
    print(comm_lp_val)
    ...
    if puh == 1.0:
        ...
        plt.savefig('results/degree_k_e.png')
        plt.clf()

    # parallel running
    directory = 'res1ks/tmpe/'
    ...
    for filename in filtered_files:
        ...
        line = file.readline()
        if sum([float(x) for x in line[1:-2].split(', ')]) != 0:
            utility_at = np.append(utility_at, np.array([float(x) for x in line[1:-2].split(', ')]).reshape((1, 4)),
                                   axis=0)
            print("###", utility_at)
        line = file.readline()
        utility_df = np.append(utility_df, np.array([float(x) for x in line[1:-2].split(', ')]).reshape((1, 2)),
                               axis=0)
        line = file.readline()
        if puh != 0.0:
            utility_u = np.append(utility_u, np.array([float(x) for x in line[1:-2].split(', ')]).reshape((1, 3)),
                                  axis=0)
        line = file.readline()
        if puh != 1.0 and not math.isinf(float(line[1:-2].split(', ')[0])):
            utility_h = np.append(utility_h, np.array([float(x) for x in line[1:-2].split(', ')]).reshape((1, 3)),
                                  axis=0)
        #polarization
        line = file.readline()
