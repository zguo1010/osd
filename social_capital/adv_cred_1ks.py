...
def initialization_capital(likers, mu, su):
    ...
    CC = likers['category'] / max(likers['category']) + likers['num_urls'] / max(likers['num_urls']) + likers[
        'num_mentions'] / max(likers['num_mentions'])
    CC = 3 * CC / max(CC)
    # CC.head()
    RC = likers['total_posts'] / max(likers['total_posts']) + likers['total_replies'] / max(likers['total_replies'])
    RC = 3 * RC / max(RC)  # 'favorite_tweets' has 2633 zeros
    ...
    capital.loc[ld, 'inviting'] = likers.loc[ld, 'friends'] / np.percentile(likers.loc[ld, 'friends'], 85)
    capital.loc[fd, 'inviting'] = likers.loc[fd, 'friends'] / np.percentile(likers.loc[fd, 'friends'], 98)
    ...
    capital.loc[capital['competence'] < 1, 'competence'] = 1
...
def main(argv):
    ...
    print('legit user',sum(likers['label'] == 'Legit'))
    print('fake user',sum(likers['label'] == 'Fake'))

    for node in likers.index.values:
        if likers.loc[node, 'label'] == 'Fake' and likers.loc[node, 'friends'] > 8000:
            likers.loc[node, 'friends'] = 8000 * 2
        elif likers.loc[node, 'label'] == 'Fake':
            likers.loc[node, 'friends'] = likers.loc[node, 'friends'] * 2
    likers.loc[likers['friends'] == 0, 'friends'] = 1
    likers.loc[likers['followers'] > 15000, 'followers'] = 15000
    likers.loc[likers['followers']==0, 'followers'] = 1

    # reduce friend number by scale
    scale = 0.01
    likers.loc[likers['label'] == 'Legit', 'friends'] = np.ceil(
        likers.loc[likers['label'] == 'Legit', 'friends'] * scale)
    likers.loc[likers['label'] == 'Fake', 'friends'] = np.ceil(
        likers.loc[likers['label'] == 'Fake', 'friends'] * scale)

    # normalize large values
    maximum_cap(likers, 'age', 0.05)#515
    maximum_cap(likers, 'lines', 0.01)#11
    maximum_cap(likers, 'category', 0.005)#214
    maximum_cap(likers, 'num_urls', 0.03)#246
    ...
    likers.loc[likers['total_posts'] > 600, 'total_posts'] = 600
    likers.loc[likers['total_replies'] > 600, 'total_replies'] = 600
    ...
    '''# drawing full graph: plot SC HC CC for 9161 nodes
    ...
    # outside i iterations/ calculate mean for repeats
    for j in range(100,attack):
    ...
    with open('sir_k_'+ out +'.txt', 'w') as f:
        for item in attack_list:
            f.write("%s\n" % item)

    with open('std_k_'+ out +'.txt', 'w') as f:
        for item in std_list:
            f.write("%s\n" % item)

    with open('state_k_'+ out +'.txt', 'w') as f:
        for item in state_list:
            f.write("%s\n" % item)

    with open('stats_k_'+ out +'.txt', 'w') as f:
        for x1 in range(100, attack):
